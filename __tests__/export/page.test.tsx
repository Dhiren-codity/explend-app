...
vi.mock('./app/export/lib/actions', (): Record<string, unknown> => {
  return {
    getCachedAuthSession: (...args: unknown[]): unknown => {
      return (globalThis as unknown as { __mocks: Record<string, unknown> }).__mocks.getCachedAuthSession(...args);
    },
    getCachedAllTransactions: (...args: unknown[]): unknown => {
      return (globalThis as unknown as { __mocks: Record<string, unknown> }).__mocks.getCachedAllTransactions(...args);
    },
    getTransactionsForExport: (...args: unknown[]): unknown => {
      return (globalThis as unknown as { __mocks: Record<string, unknown> }).__mocks.getTransactionsForExport(...args);
    },
  };
});

vi.mock('./app/export/ui/sidebar/with-sidebar', (): Record<string, unknown> => {
  const WithSidebar = ({ contentNearby }: { contentNearby: unknown }): unknown =>
    React.createElement('div', { 'data-testid': 'with-sidebar' }, contentNearby as React.ReactNode);
...
vi.mock('./app/export/ui/home/export-transactions', (): Record<string, unknown> => {
  const ExportTransactions = (props: Record<string, unknown>): unknown => {
    (globalThis as unknown as { __mocks: Record<string, unknown> }).__mocks.lastExportProps = props;
    const count =
      Array.isArray(props.transactions as unknown[]) ? (props.transactions as unknown[]).length : -1;
    return React.createElement('div', { 'data-testid': 'export-transactions', 'data-count': count });
  };
...
describe('Tests', (): void => {
  ...
  describe('Page', (): void => {
      (globalThis as unknown as { __mocks: MockFns }).__mocks = {
        getCachedAuthSession: vi.fn().mockResolvedValue({ user: { email: 'user@example.com' } }),
    ...
    test('renders ExportTransactions when transactions exist and onExport calls server action with correct args', async (): Promise<void> => {
      ...
      mocks.getCachedAllTransactions.mockResolvedValueOnce(sampleTransactions);
      mocks.getCachedAllTransactions.mockResolvedValueOnce(sampleTransactions);
    ...
    test('propagates error when getCachedAllTransactions rejects', async (): Promise<void> => {
      ...
      mocks.getCachedAllTransactions
        ...
        .mockRejectedValueOnce(new Error('fail-transactions'));
